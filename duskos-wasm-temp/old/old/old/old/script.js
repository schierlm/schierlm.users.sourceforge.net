...
let lastTimestamp = 0;
const jitWasmBuilder = { data: new Uint8Array(16), offs: 0, exports: [], startOffs: [], funcnames: [], failed: false, revision: 1, dupes: 0, pending: {} };
...
function buildJitWasm(firstFunc, funccount) {
	...
	for (let i = 0; i < funccount; i++) {
		exportSectionBodyLength += LEB128(i).length;
	}
	const exportSectionLength = LEB128(exportSectionBodyLength + funccountL.length);
	const arr = new Uint8Array(30 + funccountL.length + functionSectionLength.length + funccount + 1 + exportSectionLength.length + funccountL.length + exportSectionBodyLength + 1 + funccountL.length + codeSectionLength.length + jitWasmBuilder.offs - startOffs);
	arr.set([
		0, "a".charCodeAt(0), "s".charCodeAt(0), "m".charCodeAt(0), // signature
		1, 0, 0, 0, // version
		1, 7, 1, // type section of size 7 containing 1 type
		0x60, 2, 0x7f, 0x7f, 1, 0x7f, // function (u32, u32)
		2, 10, 1, // import section of size 10 containing 1 import
		1, "x".charCodeAt(0), 3, "m".charCodeAt(0), "e".charCodeAt(0), "m".charCodeAt(0), // name "x.mem"
		2, 0, 1, // memory with minimum size 1 page
		3  // function section of length <FUNCCOUNT>+1 with <FUNCCOUNT> functions
	], 0);
	arr.set(functionSectionLength, 30);
	let offs = 30 + functionSectionLength.length;
	arr.set(funccountL, offs); offs += funccountL.length,
		arr.fill(0, offs, offs + funccount); // all functions use the same type
	offs += funccount;
	arr[offs] = 7; // export section of size <FUNCCOUNT> * 11 + <LEN(FUNCCOUNT)> containing <FUNCCOUNT> exports
	offs++;
...
function updateJitWasm() {
	...
	WebAssembly.compile(arr).then(module => {
		WebAssembly.instantiate(module, { "x": { "mem": wasm.exports.memory } }).then(instance => {
			for (var name in instance.exports) {
				if (name.startsWith("f")) {
					jitWasmBuilder.exports[+name.substring(1)] = instance.exports[name];
				}
			}
