...
const REGT = 5; // in sync with kernel.ts
...
function doJit(where, mem) {
    ...
    // TODO: when this is removed, in< reads garbage from fmt.fs starting at byte 512
    if (getWordName(where) == "INPTR" || getWordName(where) == "in<")
        return "No idea why I may not JIT this";
    ...
    while (pendingOffsets.length > 0) {
        ...
        switch (mem[where + offset]) {
            ...
            case vmop_CELL:
                ...
                jitFunctionNeverReturns[where + (4-(where%4)&3)] = true;
